  ...
  return (
    <main className="page bills-page">
      ...
      {!loading && bills.length === 0 ? (
        <EmptyState 
          title="All clear!" 
          message="No upcoming bills found. You're either very organized or just started. Add a bill to keep track."
          icon="✅"
        />
      ) : (
        <section className="bill-list">
          {bills.map((bill) => (
            <div className={`bill-card ${statusClass(bill)}`} key={bill._id}>
              <div><h2>{bill.title}</h2><p>Due {new Date(bill.dueDate).toLocaleDateString('en-IN')}</p></div>
              <strong>₹{bill.amount.toLocaleString('en-IN')}</strong>
              <button onClick={() => updateBill(bill._id, { isPaid: true }).then(load)} disabled={bill.isPaid}>{bill.isPaid ? 'Paid' : 'Mark as paid'}</button>
              <button className="icon-button danger-text" onClick={() => deleteBill(bill._id).then(load)}>×</button>
            </div>
          ))}
        </section>
      )}
