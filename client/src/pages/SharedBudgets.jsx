...
export default function SharedBudgets() {
  ...
  const load = async () => setBudgets(await getSharedBudgets());
  useEffect(() => { load(); }, []);
  ...
  return (
    <main className="page shared-page">
      ...
      <section className="card-grid">{budgets.map((budget) => <div className="shared-card" key={budget._id}><div className="shared-head"><h2>{budget.month}</h2><span className={`badge ${budget.role}`}>{budget.role === 'owner' ? 'Owner' : 'Viewer'}</span></div><p>₹{budget.spent.toLocaleString('en-IN')} used of ₹{budget.budgetLimit.toLocaleString('en-IN')}</p><ProgressBar value={budget.percentageUsed} /><strong>{budget.percentageUsed}% used</strong></div>)}</section>
    </main>
  );
}
